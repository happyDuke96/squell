...
public class ColumnConstraintsTest {
    ...
    /// An unquoted `fullName` gets folded by H2 (`DATABASE_TO_LOWER=TRUE`) to `fullname` — same
    /// as Postgres folding an unquoted camelCase column to lowercase. `field.name()` stays
    /// `fullName` (the Java accessor's case), so the match has to be case-insensitive or this
    /// reports a false "missing column".
    @Test
    public void unquotedCamelCaseColumnFoldedByTheDatabaseStillMatches() throws SQLException {
        PostgresClient client = clientFor("""
                CREATE TABLE authors (
                    id UUID PRIMARY KEY,
                    fullName VARCHAR NOT NULL
